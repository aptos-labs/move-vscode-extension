...
use crate::types::render::{TypeRenderer, TypeRendererConfig};
...
impl Ty {
    pub fn new_ty_var(ty_var_index: &TyVarIndex) -> Ty {
        Ty::Infer(TyInfer::Var(TyVar::new_anonymous(ty_var_index.inc())))
    }

    pub fn new_ty_var_with_origin(tp_origin_loc: SyntaxLoc, ty_var_index: &TyVarIndex) -> Ty {
        Ty::Infer(TyInfer::Var(TyVar::new_with_origin(
            tp_origin_loc,
            ty_var_index.inc(),
        )))
    }
...
impl TypeFoldable<Ty> for Ty {
    ...
    fn deep_fold_with(self, folder: impl TypeFolder) -> Ty {
        match self {
            ...
            Ty::Reference(ty_ref) => Ty::Reference(ty_ref.deep_fold_with(folder)),
    ...
    fn deep_visit_with(&self, visitor: impl TypeVisitor) -> bool {
        match self {
            ...
            Ty::Reference(ty_ref) => ty_ref.deep_visit_with(visitor),
