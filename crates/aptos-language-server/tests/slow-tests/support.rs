...
use test_utils::testdir::TestDir;
use test_utils::tracing::init_tracing_for_test;
use vfs::AbsPathBuf;

pub(crate) struct Project {
    test_packages: Vec<TestPackageFiles>,
...
impl Project {
    pub(crate) fn with_test_packages(fixture: Vec<TestPackageFiles>) -> Project {
        Project {
            test_packages: fixture,
            tmp_dir: None,
            roots: vec![],
    ...
    pub(crate) fn server_with_lock(self, config_lock: bool) -> Server {
        ...
        INIT.call_once(init_tracing_for_test);

        fixtures::prepare_directories(tmp_dir.path(), self.test_packages);
...
pub(crate) fn project(test_packages: Vec<TestPackageFiles>) -> Server {
    unsafe {
        env::set_var("APT_LOG_SHOW_EMPTY_SPANS", "1");
    }
    Project::with_test_packages(test_packages).server()
}
